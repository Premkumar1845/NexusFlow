...
How a single prompt becomes a streamed, persisted result:
...
sequenceDiagram
    ...
    U->>T: Enter prompt + pick model
    T->>DB: persist(session, status=running)
    T->>H: run(model, messages, maxTokens)
    H->>API: POST JSON body
    API->>API: hasOpenRouterKey? clamp maxTokens
    API->>OR: POST chat/completions (stream=true)
    OR-->>API: SSE token chunks
    API-->>H: piped SSE stream
    loop For each chunk
        H-->>T: append delta.content
        T-->>U: live token render
    end
    H->>T: full output
    T->>DB: persist(session, output, status=done)
```
...
Core types and modules that power the platform:
...
classDiagram
    ...
    class OpenRouterClient {
        +chatComplete(opts) string
        +chatStream(opts) Response
        +generateImage(prompt, model) string
        +hasOpenRouterKey() bool
    }
    class PollinationsClient {
        +generatePollinationsImage(prompt, opts) string
        +hasPollinationsKey() bool
    }
    class SessionStore {
        +listSessions() NexusSession[]
        +saveSession(session) void
    }

    Category "1" o-- "many" ModelOption
    NexusSession --> Category : tool_category
    SessionStore --> NexusSession : persists
    OpenRouterClient ..> Category : uses models
    PollinationsClient ..> NexusSession : image output
```
...
Defined in [`supabase/schema.sql`](supabase/schema.sql) with full Row-Level Security:
